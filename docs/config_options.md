...
| `fo2_source`                                         | How the starting fO2 is set. Allowed values:`auto`, `fe3fet`, `buffer`, `absolute`. See the expanded `evo.fo2_source` subsection below.                                                                                                                                                   | (drives `FO2_buffer_SET`, `FO2_SET`, `FO2_START`) | High-impact field — see expanded subsection below for full treatment.                                                                                                                                      |
...
| Value        | Required on sample                                                         | Initial fO2 from                  | Raises?                   |
| ------------ | -------------------------------------------------------------------------- | --------------------------------- | ------------------------- |
| `auto`     | any of Fe3+/FeT, dNNO, dFMQ                                                | best available in that order      | No                        |
| `fe3fet`   | Fe3+/FeT (or speciated FeO + Fe2O3)                                        | Kress & Carmichael (1991) eq. A-5 | Yes if missing            |
| `buffer`   | `dNNO` (when `fo2_buffer: NNO`) or `dFMQ` (when `fo2_buffer: FMQ`) | Frost (1991) NNO or FMQ at T, P   | Yes if missing            |
| `absolute` | —                                                                         | `fo2_start` (bar)               | Yes if `fo2_start <= 0` |

#### `auto`

The wrapper picks the best available redox indicator on the sample, in priority order: Fe3+/FeT → `dNNO` → `dFMQ` → `evo.fo2_buffer` with offset 0. Each of the first three paths is logged at INFO. The final fallback (no redox indicator on the sample) is logged at WARNING so it is visible at any log level. Multiple indicators on the sample do not conflict — Fe3+/FeT wins.

#### `fe3fet`

Forces the Fe3+/FeT path. EVo computes initial logfO2 from Fe3+/FeT via Kress & Carmichael (1991) appendix eq. A-5 (controlled by `evo.fo2_model: kc1991`, the default). Speciated `FeO + Fe2O3` on the sample is converted to Fe3+/FeT first. Raises `ValueError` if neither `Fe3FeT` nor speciated iron is on the sample.

#### `buffer`

Forces a buffer-offset path. `evo.fo2_buffer` selects which buffer (`FMQ`, `NNO`, or `IW`); the wrapper requires the matching offset on the sample (`dFMQ` if `fo2_buffer: FMQ`; `dNNO` if `fo2_buffer: NNO`). EVo computes initial logfO2 from the offset via Frost (1991) at T and P (controlled by `evo.fmq_model: frost1991`, the default). `IW` is allowed but uses offset 0 since there is no `comp.dIW` field. Raises `ValueError` if `fo2_buffer` is `FMQ` or `NNO` and the matching offset is missing on the sample.

#### `absolute`

Sets initial fO2 directly from `fo2_start` (bar) via EVo's `FO2_SET=True` mode. Side effect: the wrapper does *not* split `FeOT` into `FeO + Fe2O3` in `chem.yaml`, because EVo refuses to run with both an absolute fO2 and a speciated iron entry. Requires both `fo2_set: true` and `fo2_start > 0`; raises `ValueError` otherwise.

#### Output columns

`fo2_source` directly sets the initial values of `logfO2`, `dFMQ`, and `Fe3Fet_m`. The chosen initial fO2 then propagates through S speciation and vapor composition for the rest of the run.

#### Initialization vs. evolution

`fo2_source` only fixes the starting state. Whether fO2 evolves with pressure during degassing is gated by `evo.fe_system` (default `true`, evolves).

#### Disambiguation from `evo.fo2_model`

`fo2_source` chooses the input pathway (which sample value the wrapper reads). `evo.fo2_model` chooses the conversion equation (Fe3+/FeT ↔ logfO2) used inside the Fe3+/FeT pathway. They are independent settings: `fo2_model` is consulted whenever EVo needs to translate between Fe3+/FeT and logfO2, regardless of which path `fo2_source` selected.
