...
Anything *italicized in this doc* is a comment from the writer, not canonical documentation.
...
| `H2O` (wt%)                                                                         | Bulk water, taken in as the neutral oxide H2O (not as separate H2/H2O/CH4 melt species). Whether a backend then re-speciates it internally depends on the backend logic and on the user's COH-species toggle.        |
| `CO2` (wt%)                                                                         | Bulk CO2, taken in as neutral CO2.                                                                                                                                                                                     |
| `S` (wt%)                                                                           | Bulk sulfur, taken in as elemental S.                                                                                                                                                                                  |
| `Fe3FeT`, `dNNO`, `dFMQ`                                                        | Redox indicators. Whether a given backend uses Fe3+/FeT, dNNO, or dFMQ is controlled by a per-backend YAML knob — see each backend's "Field-by-field" table for the exact knob name and the per-backend fallback chain. |
...
Per-backend redox-indicator preference is controlled by these YAML fields: `evo.fo2_source` (auto / fe3fet / buffer / absolute); `volfe.fo2_column` (DNNO/Fe3FeT/DFMQ) plus `volfe.fo2_source` (auto / fe3fet / dnno / dfmq); `magec.redox_option` (logfO2 / dFMQ / Fe3+/FeT / S6+/ST) plus `magec.redox_source` (auto / fe3fet / dfmq / dnno / kc91_from_buffer); SulfurX has no knob — it always tries `dFMQ` first, then Fe3+/FeT (via KC91 inversion at 1 bar), then `dNNO` (via Frost-1991 buffer offset).
...
| `starting_p`                                                                                                          | `'Pvsat'` (the default) starts the run at the saturation pressure VolFe finds itself; `'set'` uses a user-supplied starting P (currently not exposed).                                                                                                                    | `starting_P`                                                                                                                                          |                                                                                                                                   |
...
When an oxide is exactly 0 in the sample, volcatenate writes `1e-10` instead, **because EVo's internal `single_cat()` silently drops keys with value 0**, and downstream functions then crash with KeyError on species like `mno`. See [backends/evo.py:283](../src/volcatenate/backends/evo.py). Result: even "absent" minor oxides are present at 1e-10 in the calculation. Effect on results is numerically negligible.
...
`_compute_delta_fmq` at [backends/sulfurx.py](../src/volcatenate/backends/sulfurx.py) is the existing clean explicit cascade — SulfurX has no `redox_source` knob; it always tries the indicators in this fixed order and logs the choice at INFO:
