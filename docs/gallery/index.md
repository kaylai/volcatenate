...
| **Scenario 2: sample with only `dNNO`, who can run it** | **stub** | Runnable Example 2. EVo and VolFe consume `dNNO` natively; MAGEC and SulfurX raise `ValueError` because they do not accept `dNNO` and the wrapper does not convert it. Shows the tracebacks and how to supply `Fe3FeT` / `dFMQ` instead. |
