...
mpl_colors = [tuple(int(x) / 255 for x in re.findall(r"\d+", rgb)) for rgb in rgb_list]
...
vp.save_plotly_fig(
    fig4,
    os.path.join(RESULTS_DIR, "Fig_P_norm_H2Om_CO2m_Sm"),
...
vp.save_plotly_fig(
    fig6,
    os.path.join(RESULTS_DIR, "Fig_P_norm_redox"),
    overwrite_if_exists=True,
    scale=4,
)
...
all_data, fig7, axes7 = vp.plot_all_redox_variables(
    systems,
    colors=["#4477AA", "#EE6677", "#6F6F6F", "#93CC44"],
    alpha=0.35,
    legend_order=legend_order,
)
for ax in axes7:
...
vp.save_plotly_fig(
    fig8a, os.path.join(RESULTS_DIR, "Fig_CS_vapor"), overwrite_if_exists=True, scale=4
)
