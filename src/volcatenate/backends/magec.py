...
def _build_sample_input_rows(
    ...
    # ── Oxides: pass through from the CSV unchanged ──
    sio2  = comp.SiO2
    tio2  = comp.TiO2
    al2o3 = comp.Al2O3
    cr2o3 = comp.Cr2O3
    ...
    # MAGEC's Bulk_H, Bulk_C, Bulk_S fields are elemental weight percent.
    # We apply only the molecular → elemental conversion that this format
    # requires; the values are otherwise passed through from the CSV.
    # Rounded molecular weights match the author convention: the published
    # Canary inputs show Bulk_H = 0.33333 and Bulk_C = 0.81818, which
    # back-calculate to clean H2O = 3.0 and CO2 = 3.0 only with rounded
    # weights (H2O = 18; H = 2; CO2 = 44; C = 12).
